import { getAirportsForCity, getAirportsForCountry } from "@/app/_shared/utils/airports";

export const isInRegion = (iata: string, region: string): boolean => {
  return Boolean(regions[region]?.has(iata.toLowerCase()));
};

const getIatasForCountries = (countryString: string): Set<string> => {
...
const getIatasForCities = (citiesString: string): Set<string> => {
...
const regions: Record<string, Set<string>> = {
  southeastAsia: getIatasForCountries(
    "Brunei, Bhutan, Cambodia, Cocos Island, Indonesia, Laos, Malaysia, Myanmar, Philippines, Singapore, Thailand, Timor Lest, Vietnam"
  ),
  northeastAsia: getIatasForCountries(
    "China, Hong Kong, Japan, Macao, Mongolia, North Korea, Republic of Korea, Taiwan"
  ),
  southeastEurope: getIatasForCountries("Greece, Turkey, Cyprus"),
  northernEurope: getIatasForCountries("Finland, Norway, Sweden"),
  westernEurope: getIatasForCountries(
  ...
  usaEastCoast: new Set([
    ...getIatasForCities(
      "Boston, Charlotte, Chicago, Miami, New York, Orlando, Toronto, Washington DC"
    ),
    "ewr",
  ]),
  usaWestCoast: getIatasForCities(
    "Las Vegas, Los Angeles, Phoenix, San Francisco, Seattle, Vancouver"
  ),
